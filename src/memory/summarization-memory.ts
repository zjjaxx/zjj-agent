...
import { type ModelWithTools } from "../utils/invoke";
...
export async function summarizationMemoryDemo(model: ModelWithTools) {
  ...
  if (totalTokens >= maxTokens) {
    ...
    const summary = await summarizeHistory(messagesToSummarize, model); // 清空历史消息，只保留最近的消息
    await history.clear();
